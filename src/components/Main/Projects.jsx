...
const Projects = () => {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {/* Filters */}
      <div className="flex h-10 w-[100%] justify-around items-center mb-4">
        {type === "development" ? null : (
          <div className="flex z-10 items-center gap-6">
            <p onClick={() => changeType("development")}>Dev</p>
            {type === "design" ? (
              <FunnelX size={10} color={"hsl(var(--accent-color))"} onClick={() => changeType("")} />
            ) : null}
          </div>
        )}
        {type === "development" ? (
          <div className="text-[12px] flex gap-4">
            <p onClick={() => setSubType("full-stack")}>Full-stack</p>
            <p onClick={() => setSubType("front-end")}>Frontend</p>
            <p onClick={() => setSubType("backend")}>Backend</p>
            <p onClick={() => setSubType("")}>all</p>
          </div>
        ) : null}
        <div
          className={`${
            type == "design"
              ? "ml-10 z-0 absolute left-0 w-[20%] h-10 border-b-1 border-r-1"
              : type === "development"
              ? "mr-10 z-0 absolute right-0 w-[25%] h-10 border-b-1 border-l-1"
              : "hidden"
          }`}
        ></div>
        {type === "design" ? null : (
        ...
        {type === "design" ? (
          <div className="text-[12px] flex gap-4">
            <p onClick={() => setSubType("graphics design")}>Graphics Design</p>
            <p onClick={() => setSubType("product design")}>Product Design</p>
            <p onClick={() => setSubType("")}>all</p>
          </div>
        ) : null}
      </div>
